    ...
    config = function()
      ...
      vim.api.nvim_create_autocmd('LspAttach', {
        ...
        callback = function(event)
          ...
          map(leader .. 'r', telescope.lsp_references, '[R]eferences')

          -- Jump to the implementation of the word under your cursor.
          --  Useful when your language has ways of declaring types without an actual implementation.
          map(leader .. 'i', telescope.lsp_implementations, '[I]mplementation')

          -- Jump to the definition of the word under your cursor.
          --  This is where a variable was first declared, or where a function is defined, etc.
          --  To jump back, press <C-t>.
          map(leader .. 'd', telescope.lsp_definitions, '[D]efinition')
          ...
          map(leader .. 's', telescope.lsp_document_symbols, 'Open Document [S]ymbols')

          -- Fuzzy find all the symbols in your current workspace.
          --  Similar to document symbols, except searches over your entire project.
          map(leader .. 'S', telescope.lsp_dynamic_workspace_symbols, 'Open Workspace [S]ymbols')

          -- Jump to the type of the word under your cursor.
          --  Useful when you're not sure what type a variable is and you want to see
          --  the definition of its *type*, not where it was *defined*.
          map(leader .. 't', telescope.lsp_type_definitions, '[T]ype Definition')
          ...
          map(leader .. 'f', vim.lsp.buf.format, '[F]ormat file', { 'n', 'x' })

          -- This mapping override the default mapping to show hover documentation with a rounded border.
          map('K', function()
            vim.lsp.buf.hover { border = 'rounded' }
          end, 'Hover Documentation')

          -- This mapping override the default mapping to show diagnostics in a floating window with a rounded border.
          -- map('[d', function()
          --   vim.diagnostic.jump { float = true, buffer = true, count = -1 }
          ...
          map(leader .. 'h', function()
            local exists = vim.fn.exists ':LspClangdSwitchSourceHeader' == 2
            if exists then
              vim.cmd 'LspClangdSwitchSourceHeader'
            end
          end, 'Switch Source/[H]eader')
          ...
          local leader_toggle = '<leader>tl'
          -- The following code creates a keymap to toggle inlay hints in your
          -- code, if the language server you are using supports them
          --
          -- This may be unwanted, since they displace some of your code
          if client and client_supports_method(client, vim.lsp.protocol.Methods.textDocument_inlayHint, event.buf) then
            map(leader_toggle .. 'h', function()
              vim.lsp.inlay_hint.enable(not vim.lsp.inlay_hint.is_enabled { bufnr = event.buf })
            end, 'Inlay [H]ints')
          end

          map(leader_toggle .. 'd', function()
            vim.cmd 'LspToggleDiagnostics'
          end, '[D]iagnostics')

          map(leader_toggle .. 'i', function()
            vim.cmd 'LspToggleInline'
          end, 'Diagnostics [I]nline Virtual Text')
        end,
      })
